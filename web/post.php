	...
	<div class="welcome">
		<div class="container">
			...
			<div class="w3ls_news_grids">
				<div class="col-md-10 w3_agile_mail_left">
					<div class="agileits_mail_grid_right1 agile_mail_grid_right1">
						<form method="post" enctype="multipart/form-data">
							...
							<span>
								<i>Tag</i>
								<input type="text" name="Tag" placeholder=" " required>
							</span>
							<span>
								<i>Admin Name</i>
              ...
              <span>
                <i>Image</i>
                <input type="file" accept="image/*" name="Image" placeholder=" " required>
              </span>
							<span>
								<i>Article</i>
								<textarea name="Article" placeholder=" " required></textarea>
							</span>
							<div class="w3_submit">
...
if(isset($_POST['submit'])){
  ...
  if(in_array($imageFileType,$extension_arr)){
    $query = "insert into tbl_artikel (title, tag, image, nama_admin, artikel, tanggal)
    values ('".$title."','".$tag."','".$name."','".$admin."','".$artikel."','".$tgl."')";
    mysqli_query($dbh,$query);

    move_uploaded_file($_FILES['Image']['tmp_name'],$target_dir.$name);
    echo "<script>alert('The Article Has Been Posted!')</script>";
